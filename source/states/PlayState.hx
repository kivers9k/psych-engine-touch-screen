...
import touchscreen.Hitbox;
import touchscreen.PauseButton;
...
#elseif (hxCodec == "2.5.1") import vlc.MP4Handler as VideoHandler;
...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		hitbox = new Hitbox();
		hitbox.hideHint = !ClientPrefs.data.hideHint;
		hitbox.visible = ClientPrefs.data.touchScreen;
	...
	override public function update(elapsed:Float)
	{
		...
		for (h in hitbox.hitboxs) {
			var index:Int = hitbox.hitboxs.indexOf(h);
			if (h.justPressed) keyPressed(index);
			if (h.justReleased) keyReleased(index);
		}
	...
	private function keysCheck():Void
	{
		...
		for (hb in hitbox.hitboxs) {
			holdArray.push(hb.pressed);
			pressArray.push(hb.justPressed);
			releaseArray.push(hb.justReleased);
		}
		...
		if (startedCountdown && !boyfriend.stunned && generatedMusic)
		{
			...
			if(notes.length > 0)
			{
				...
				{
					// hold note functions
					var longNote:Bool = holdArray[daNote.noteData] || hitbox.hitboxs[daNote.noteData].pressed;
					if (strumsBlocked[daNote.noteData] != true && daNote.isSustainNote && longNote && daNote.canBeHit && daNote.mustPress && !daNote.tooLate && !daNote.wasGoodHit && !daNote.blockHit) {
						goodNoteHit(daNote);
					}
				});
			}
